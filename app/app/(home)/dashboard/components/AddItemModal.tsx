...
export default function AddItemModal({ isModalOpen, setIsModalOpen }: Props) {
  const { ID, selection, setID, postItem } = useSelection();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // post request per aggiungere l'item (id, tag, website, username, password) 
    // id è il prossimo id assegnabile e va assegnato come fatto in folders
    setID((parseInt(ID) + 1).toString());
    const newItem: Item = {id: ID, tag: tag, website: website, username: username, password: password, securityLevel: "medium", 
      folderID: (selection === "All Items" || selection === "Password Health") ? "0" : selection}
    postItem(newItem);
    console.log({ tag, website, username, password });
    closeModal();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30 px-4">
      ...
      >
        ...
        <form className="flex flex-col gap-4 min-w-[280px]" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="mt-4 bg-blue-100 text-black py-2 rounded hover:bg-blue-200 transition-colors font-semibold"
