...
export default function PasswordItem({ tag, type, folder, securityLevel }: Props){
        ...
        <div 
            ...
            >
                <div className="flex items-center justify-between w-full">
                    ...
                    <label className="text-gray-600 w-1/4">{type}</label>
                    <label className="text-gray-500 text-sm w-1/4">{folder}</label>
                    <div className="flex items-center gap-1 w-1/4">
                        {getSecurityColors().split(' ').map((color, index) => (
                            <div 
                                key={index}
                                className={`w-12 h-1 ${color}`}
                            />
                        ))}
                    </div>
                </div>
